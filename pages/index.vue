...
<template>
  ...
  >
    <section
      class="flex h-screen flex-col justify-center px-4 text-center sm:px-[15%]"
    >
      <TextTitle class="text-6xl">Anci</TextTitle>
      <em
        class="font-serif text-3xl font-bold not-italic text-primary-light dark:text-primary-dark"
      >
    ...
    <section>
      ...
      <div class="relative overflow-x-hidden rounded-3xl">
        <button
          class="highlight-none group absolute left-0 hidden h-full items-center sm:flex"
          aria-label="scroll left"
          type="button"
          @click="scrollGallery(-scrollSpeed)"
        >
          <Icon
        ...
        <button
          class="highlight-none group absolute right-0 hidden h-full items-center sm:flex"
        ...
        <div
          class="my-11 flex h-[400px] w-full items-center gap-x-3 overflow-x-auto px-3 sm:overflow-x-hidden"
          v-if="!loadingGallery"
          ref="galleryDiv"
        >
          <img
            class="block max-h-[100%] max-w-[calc(70vw)] rounded-3xl"
            v-for="{
              photo: {
                id: pictureId,
                originalHeight,
